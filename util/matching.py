    ...
    while set1_stop_len < set1_finish and set2_stop_len < set2_finish:
        ...
        try:  # 0
            temp.append(cosine_similarity(
                master_data[set1_index], slave_data[set2_index]))
        except:
            temp.append(0)
        for size in range(1, window_size):
            try:  # 1
                temp.append(cosine_similarity(
                    master_data[set1_index+size], slave_data[set2_index]))
            except:
                temp.append(0)
        ...
        for size in range(1, window_size):
            try:  # 3
                temp.append(cosine_similarity(
                    master_data[set1_index], slave_data[set2_index+size]))
            except:
                temp.append(0)
        ...
        if max_index == 0:
            ...
            for size in range(1, window_size):
                try:  # 1
                    C1 = cosine_similarity(
                        master_data[set1_index+max_index], slave_data[set2_index+size])
            ...
            for size in range(1, window_size):
                try:  # 1
                    C1 = cosine_similarity(
                        master_data[set1_index+size], slave_data[set2_index+max_index])
                    # C1 = 1 - spatial.distance.cosine(master_data[set1_index+1], slave_data[set2_index+1])
                except:
                    C1 = 0
                compare_cosine.append(C1)
...
def sets_matching(master, slave):
    ...
    master_sets_length, slave_sets_length = len(
        used_master_index), len(used_slave_index)
    ...
    if master_sets_length == 0 and slave_sets_length == 0:
        ...
        set1_result, set2_result = DP_matching(
            embed_master_data, embed_slave_data, window_size)

    collect_no_match_vec = []  # 用來收集對齊到空的那些set的向量表示
    # 用來表示每個set有幾個對齊到空的數量 , for example, [2,3,4] 表示page1 有兩個set對到空 page2有3個...
    collect_no_match_num = []
    ...
    for i in range(len(set2_result)):
        ...
        if set1_result[i] == 0:  # 表示page2的set2_result[i] 對齊到空集 要將資料額外取出來 最後再統整時重算相似度
            # page2的set2_result[i] 的向量表示, set2_result[i]表示在page2第i個set
            collect_no_match_vec.append(embed_slave_data[set2_result[i]-1])
    ...
    for master_index, slave_index in set_result.items():
        ...
        if master_index == 0:
            ...
            if slave_index[0] < 0:
                ...
                df = pd.DataFrame(
                    arr_t, columns=['col'+str(item) for item in range(0, len(arr_t.T))])

                arr_p = np.array(sets_predict)

                df_predict = pd.DataFrame(
                    arr_p, columns=['col'+str(item) for item in range(0, len(arr_p.T))])
            ...
            if 1 in label_:
                group_vote = cal_vote_result_for_KNN(
                    my_model, column_num, testing_row_number, embeddings_predict)
            ...
            for i in range(len(predict[predict_index[slave_index[0]-1]])):
                ...
                for x in column_result:
                    ...
                    temp_arr.append(
                        predict[predict_index[slave_index[0]-1]][i][x])
